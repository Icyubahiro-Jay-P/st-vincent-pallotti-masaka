...
function ToastFromQuery() {
  ...
  React.useEffect(() => {
    if (!key) return
    const entry = TOAST_MESSAGES[key as ToastKey]
    if (entry) toast[entry.type](entry.message)

    // Strip the one-shot signal so a refresh doesn't replay the toast, and
    // use replace (not a full navigation) so it doesn't add a history entry.
