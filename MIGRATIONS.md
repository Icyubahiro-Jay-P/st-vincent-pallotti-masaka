...
safe for _both_ the old and new app code to run against at once.
...
1. **Expand** ship an additive-only migration (new nullable column, new
   table, new index) _before_ any app code depends on it. Run
...
3. **Contract** only once you're confident you won't roll back past the
   expand step, ship a _separate_ migration that drops/renames/tightens
