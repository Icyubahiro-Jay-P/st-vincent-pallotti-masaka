...
import { programs, programHref, localeHref, type ProgramSlug } from "@/lib/site-config"
...
export function generateStaticParams() {
  return locales.flatMap((locale) =>
    detailPrograms.map((program) => ({ locale, slug: program.slug }))
  )
}
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-10 px-4 sm:px-6 lg:grid-cols-3 lg:px-8">
          <div className="flex flex-col gap-4 lg:col-span-1">
            ...
            <Button
              render={
                <Link
                  href={`${localeHref(locale, "/admissions")}?program=${encodeURIComponent(text.name)}`}
                />
              }
          ...
          <div className="mt-8 grid grid-cols-1 gap-px overflow-hidden border border-border bg-border sm:grid-cols-2 lg:grid-cols-3">
            {otherPrograms.map((other) => {
              ...
              return (
                <Link
                  key={other.slug}
                  href={programHref(locale, other.slug as ProgramSlug)}
